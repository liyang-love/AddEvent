...
class ReportHead extends React.PureComponent<ReportSpace.HeadHQ & ReportSpace.HeadWrapProp   , ReportHeadState> {
	...
	render() {
		...
		const { patientName, age, medicalRecordNumber,
			 ...
			 currentPeople, cpProfession, cpOrgId,job,
			reporter,   dadIncidentSceneId, reporterNumber, incidentSceneId, happenTime, reportTime, patientOrgId, 
		} = parmas;
		...
		return (<>
			<div className="item-tr">
				...
				<span className="detail">
					<label >年龄：<input type="number" name="age" onChange={inputChange} required defaultValue={age} className={!!age ? "inp" : "inp no-fill"} style={{ width: 60 }} /></label>
				</span>
				<span className="detail">
					<label className="require">病历号/病案号：<input type="text" placeholder="非必填" name="medicalRecordNumber" defaultValue={medicalRecordNumber} onChange={inputChange} className={medicalRecordNumber ? "inp" :"inp no-fill"} style={{ width: "120px" }} /></label>
				</span>	
                <span className="detail">
					<span >所在科室：</span>
					<ComTreebox data={orgArr} clickCallback={setComboboxObj} textFiled="name" defaultSel={patientOrgId + ""} filed="patientOrgId" hasSlideIcon={false} width={130} pannelWidth={300} />
				</span>
			</div>
			<div className="item-tr">
