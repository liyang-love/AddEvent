...
class Head extends React.PureComponent<RouteComponentProps<HeadProp> & reduxStateProp & dispatchProp, HeadState>{
	...
	submitPassword(){
		...
		Api.updatePwd({
			...
			password,
			userName:""
		}).then((res:AxiosInterfaceResponse)=>{
			...
			if(res.code==200){
				...
				notifition.addNotice(res.message,"warn");
			}
...
const mapStateToProps: MapStateToProps<reduxStateProp, RouteComponentProps<HeadProp>, appStore> = ({ app }) => {
	...
	return {
		user_name: app.get("userName"),
		role_arr: app.get("roleName"),
		role_ids: app.get("roleId"),
		role_index: app.get("roleIndex"),
		user_id:app.get("id"),
	}
